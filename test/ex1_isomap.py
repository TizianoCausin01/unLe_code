...
C = X @ X.T
print(C.shape)
eval, evec = eigh(C, subset_by_index=(0, 9))
...
# %%
GG_dcnt = unLe_package.double_centering(G, 1e-5)

# %%
aa = any(abs(np.sum(GG_dcnt, axis=0)) > 1e-5)
print(aa)
# %%
...
# %%
eval_dG, evec = eigh(GG_dcnt)
plt.scatter(np.arange(1000), eval_dG)
plt.show()
# %%
print(np.linalg.matrix_rank(GG_dcnt))
print(eval_G[990:999])
